...
from run_mrph_timesnet_670 import set_random_seed
from utils.metrics import metric

OUTPUT_DIR = Path("results/dl_result/res_all/sensitivity_analysis_670")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="One-factor-at-a-time sensitivity analysis for MRPH-TimesNet physical and gate parameters on RF670."
    )
    ...
    parser.add_argument("--data_path", type=str, default="water_timeseries_670.csv", help="dataset file")
    ...
    parser.add_argument("--seed", type=int, default=488, help="reproducibility seed")
...
def plot_sensitivity(
    ...
    for idx, parameter_name in enumerate(parameter_names):
        ...
        ax.plot(subset["value"], subset["test_nse"], marker="o", linewidth=1.8, color="#d62728", label="Test NSE")
        ax.axvline(baseline_row["value"], linestyle="--", linewidth=1.0, color="black", alpha=0.8)
        ax.scatter(
            [baseline_row["value"]],
        ...
        ax.text(baseline_row["value"], y_max + y_pad * 0.15, "Learned", ha="center", va="bottom", fontsize=8)
        ax.annotate(
...
def run_sensitivity_analysis() -> pd.DataFrame:
    ...
    catalog_csv_path = safe_to_csv(pd.DataFrame(parameter_catalog_rows), output_dir / "mrph_parameter_catalog_670.csv")
    ...
    results_csv_path = safe_to_csv(results_df, output_dir / "mrph_parameter_sensitivity_670.csv")
    ...
    summary_csv_path = safe_to_csv(pd.DataFrame(summary_rows), output_dir / "mrph_parameter_sensitivity_summary_670.csv")
    plot_sensitivity(
        results_df,
        output_dir,
        PHYSICAL_PARAMETERS,
        "mrph_physical_parameter_sensitivity_670",
        "One-Factor-at-a-Time Sensitivity of MRPH-TimesNet Physical Parameters on RF670",
    )
    plot_sensitivity(
        results_df,
        output_dir,
        GATE_PARAMETERS,
        "mrph_gate_parameter_sensitivity_670",
